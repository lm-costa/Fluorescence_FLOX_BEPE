...
correl[,4] <- as.numeric(spect_tab$spect)
...
pear_resul <- vector("list", length = ncol(tab2))
for(i in 1:length(pear_resul)){
  pear_resul[[i]] <- cor.test(tab2$gpp/df$aPAR, tab2[,i]/df$aPAR)
}
...
correl_fy[,4] <- as.numeric(spect_tab$spect)
...
a = dplyr::case_when(
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(22))~ 81.36 - 13.45*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(24)) ~ 78.61 -10.49*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(26)) ~ 77.05 -8.78*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(28)) ~ 77 -8.727*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(30)) ~ 75.157 -6.637*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(32)) ~ 73.526 -4.753*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(34)) ~ 72.087 -3.057*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(36)) ~ 70.665 -1.342*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(38)) ~ 68.9615 +0.7643*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(40)) ~ 67.24 +2.949*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(42)) ~ 65.552 +5.152*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(44)) ~ 63.412 +8.033*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) <=cos(circular::rad(46)) ~ 61.39 +10.84*tab$Incoming750_Wm2nm1sr1,
  abs(cos(circular::rad(fobs$SZA))) > cos(circular::rad(46)) ~ 59.58 +13.45*tab$Incoming750_Wm2nm1sr1,
)

b = -0.23*(tab$Incoming750_Wm2nm1sr1)^2 + 0.58*tab$Incoming750_Wm2nm1sr1 + 0.43
...
correl_fr[,4] <- as.numeric(spectra_Table$spectra)



pear_resul <- vector("list", length = ncol(tab2))
for(i in 1:length(pear_resul)){
  pear_resul[[i]] <- cor.test(df_frc$LUE, tab2[,i]/df$aPAR)
}

for(i in 1:length(pear_resul)){
  if(i==1){
    correl_fyc <- round(pear_resul[[i]]$estimate,3)
    lower <- pear_resul[[i]]$conf.int[1]
    uper <- pear_resul[[i]]$conf.int[2]
    correl_fyc <- cbind(correl_fyc, lower,uper)
  }else{
    correl_afyc <- round(pear_resul[[i]]$estimate,3)
    lower_a <- pear_resul[[i]]$conf.int[1]
    uper_a <- pear_resul[[i]]$conf.int[2]
...
correl_fyc[,4] <- as.numeric(spectra_Table$spectra)
...
correl[-c(683,684),] |>
  ...
  ggplot2::geom_ribbon(ggplot2::aes(ymax=correl_fr$uper[-c(683,684)],
                                    ymin=correl_fr$lower[-c(683,684)]),
                       color='red', linetype='dashed',alpha=.1)+
  ggplot2::geom_point(ggplot2::aes(x=correl_fr$V4[-c(683,684)],
                                   y=correl_fr$correl_fr[-c(683,684)]),
                      color='red')+
  ggplot2::ylim(0,1)+
...
correl_fy[-c(683,684),] |>
  ...
  ggplot2::geom_ribbon(alpha=.1,linetype="dashed",color="black")+
  ggplot2::geom_jitter(color="black")+
  ggplot2::geom_ribbon(ggplot2::aes(ymax=correl_fyc$uper[-c(683,684)],
                                    ymin=correl_fyc$lower[-c(683,684)]),
                       color='red', linetype='dashed',alpha=.1)+
  ggplot2::geom_point(ggplot2::aes(x=correl_fyc$V4[-c(683,684)],
                                   y=correl_fyc$correl_fyc[-c(683,684)]),
                      color='red')+
  ggplot2::ylim(.9,1)+
  ggplot2::labs(x=expression("Wavelenght ("~ lambda ~")"),
...
df_frc_clear <- readr::read_rds("data/partial/df_frc.rds")


for(i in 3:684){
  if(i==3){
    dif = df_frc_clear[,i]-df_frc[,i]
  }else{
    dif_a = df_frc_clear[,i]-df_frc[,i]
    dif <- rbind(dif,dif_a)
  }
}

dif <- dif |> t() |> data.frame()
names(dif) <- names(df_frc)[3:684]
dif <- cbind(df_frc$UTC, df_frc$day,dif)
names(dif)[1:2] <- c("UTC","day")

dif |>
  tidyr::pivot_longer(
    cols = "669.98":"779.96",
    names_to = "wavelength",
    values_to = "Fluorescence Diference") |>
  dplyr::filter(day!=155) |>
  ggplot2::ggplot(ggplot2::aes(x=as.numeric(wavelength), y=`Fluorescence Diference`,
                               color=lubridate::hour(UTC))) +
  ggplot2::geom_point() +
  ggplot2::geom_line()+
  ggplot2::facet_wrap(~day)+
  ggplot2::labs(x="Wavelength", color="Hour")+
